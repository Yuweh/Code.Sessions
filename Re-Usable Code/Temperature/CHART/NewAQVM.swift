    ...
    public func generateChartData(from: NSNumber, to: NSNumber) {
        ...
        DispatchQueue.global(qos: .utility).async {
            if let macAddress = self.sensorData.value?.sensorMacAddress {
                ...
                let propertiesToFetch: [String] = [TktConstants.Key.attrTimestamp, TktConstants.Key.attrTemperature, TktConstants.Key.attrHumidity, TktConstants.Key.attrCO2, TktConstants.Key.attrVOC]
